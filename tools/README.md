...
| `CommentResidue/` | Scans `src/`, `tests/`, and `tools/` for forbidden comment/string content against `patterns.txt`, ratcheted by the committed `baseline.txt`. Run by the `comment-residue` CI job on every push and pull request; part of `AcDream.slnx`, `tests/AcDream.Tools.CommentResidue.Tests` covers it. |
