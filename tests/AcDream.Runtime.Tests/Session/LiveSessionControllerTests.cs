...
{
    [Fact]
    public void PolledConnectPublishesRosterOnlyAfterCompletionOnTickThread()
    {
        ...
        host.OnConnected = () => Assert.Equal(ownerThread, Environment.CurrentManagedThreadId);
        var options = LiveOptions(awaitSelection: true) with { PollConnectionDuringTicks = true };
        Assert.Equal(LiveSessionStartStatus.Deferred, controller.Start(options, host).Status);
        Assert.Equal(RuntimeConnectionStatus.Connecting, controller.Connection.Snapshot.Status);
        Assert.Equal(LiveSessionStartStatus.Deferred, controller.Start(options, host).Status);
        controller.Tick();
        Assert.Equal(0, operations.TickCount);
        Assert.Equal(RuntimeCharacterSelectionLifecycle.Connecting,
            controller.CharacterSelection.Snapshot.Lifecycle);

        operations.ConnectionComplete = true;
        controller.Tick();
        Assert.Equal(RuntimeConnectionStatus.Ready, controller.Connection.Snapshot.Status);
        Assert.Equal(RuntimeCharacterSelectionLifecycle.AwaitingSelection,
            controller.CharacterSelection.Snapshot.Lifecycle);
        Assert.Equal(0, operations.EnterWorldCount);
    }

    [Fact]
    public void PolledConnectFailureStaysVisibleAndCannotAutoEnter()
    {
        var calls = new List<string>();
        var operations = new TestOperations(calls)
        {
            OnPollConnect = () => throw new UnsupportedDataUpdateException(),
        };
        using var controller = new LiveSessionController(operations);
        controller.Start(LiveOptions() with { PollConnectionDuringTicks = true }, new TestHost(calls));
        controller.Tick();
        Assert.Equal(RuntimeConnectionStatus.Unsupported, controller.Connection.Snapshot.Status);
    ...
    [Fact]
    public void PolledUnsupportedUpdateKeepsPrimaryErrorWhenCleanupNeedsRetry()
    {
        var calls = new List<string>();
        var operations = new TestOperations(calls)
        {
        ...
        controller.Tick();
        Assert.Equal(RuntimeConnectionStatus.Unsupported, controller.Connection.Snapshot.Status);
        Assert.Equal(new UnsupportedDataUpdateException().Message, controller.Connection.Snapshot.Error);
        Assert.True(controller.CaptureOwnership().HasRetiredSession);
        controller.Stop();
        Assert.False(controller.CaptureOwnership().HasRetiredSession);
        Assert.Equal(1, operations.DisposeCounts[session]);
    }

    [Fact]
    public void StoppedPolledConnectNeverContinuesAndNewAttemptCanComplete()
    {
        var calls = new List<string>();
        var operations = new TestOperations(calls);
        using var controller = new LiveSessionController(operations);
        var host = new TestHost(calls);
        var options = LiveOptions(awaitSelection: true) with { PollConnectionDuringTicks = true };
        controller.Start(options, host);
        WorldSession oldSession = controller.CurrentSession!;
        controller.Stop();
        operations.ConnectionComplete = true;
        controller.Tick();
        Assert.Equal(0, operations.PollConnectCount);
        Assert.Equal(RuntimeConnectionStatus.Inactive, controller.Connection.Snapshot.Status);
        controller.Start(options, host);
        controller.Tick();
        Assert.NotSame(oldSession, controller.CurrentSession);
        Assert.Equal(RuntimeConnectionStatus.Ready, controller.Connection.Snapshot.Status);
        Assert.Equal(1, operations.DisposeCounts[oldSession]);
    }

    [Fact]
    public void UnsupportedDataUpdatePersistsAfterCleanupAndReconnectResetsIt()
    {
        var calls = new List<string>();
        var operations = new TestOperations(calls);
        using var controller = new LiveSessionController(operations);
        var host = new TestHost(calls);
        operations.OnConnect = () => throw new UnsupportedDataUpdateException();
        LiveSessionStartResult failed = controller.Start(LiveOptions(), host);
        Assert.Equal(LiveSessionStartStatus.Failed, failed.Status);
        Assert.Equal(RuntimeConnectionStatus.Unsupported, controller.Connection.Snapshot.Status);
        Assert.Contains("does not support", controller.Connection.Snapshot.Error);
        Assert.Null(controller.CurrentSession);
        Assert.Equal(0, operations.EnterWorldCount);

        operations.OnConnect = () => Assert.Equal(RuntimeConnectionStatus.Connecting,
            controller.Connection.Snapshot.Status);
        controller.Start(LiveOptions(), host);
        Assert.Equal(RuntimeConnectionStatus.Ready, controller.Connection.Snapshot.Status);
        Assert.Null(controller.Connection.Snapshot.Error);
        controller.Stop();
        Assert.Equal(RuntimeConnectionStatus.Inactive, controller.Connection.Snapshot.Status);
    }
    ...
    private sealed class TestOperations(List<string> calls) : ILiveSessionOperations
    {
        ...
        public Action? OnPollConnect { get; set; }
        public bool ConnectionComplete { get; set; }
        public int PollConnectCount { get; private set; }
