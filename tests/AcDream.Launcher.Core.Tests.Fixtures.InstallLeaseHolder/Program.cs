...
return effectiveArgs.FirstOrDefault() switch
{
    ...
    "hold-launcher-session-process" =>
        await HoldLauncherSessionProcessAsync(effectiveArgs[1..]),
    _ => 2,
};
...
static async Task<int> HoldLauncherSessionProcessAsync(string[] arguments)
{
    if (arguments.Length != 4
