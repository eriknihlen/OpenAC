...
{
    [Fact]
    public async Task ZeroTimeoutPollReturnsWithoutNetworkInput()
    {
        using var client = new NetClient(new IPEndPoint(IPAddress.Loopback, 1));
        Task<int> poll = Task.Run(() => client.Receive(new byte[32], TimeSpan.Zero, out _));
        Assert.Equal(-1, await poll.WaitAsync(TimeSpan.FromSeconds(2)));
    }
