...
{
    [Fact]
    public void DirectCharacterStartupUsesSavedSizeWithOrdinaryWindowDecoration()
    {
        WindowOptions options = GameWindow.CreateStartupWindowOptions(false, "1760x990", true,
            directCharacterLaunch: true);
        Assert.Equal(new Vector2D<int>(1760, 990), options.Size);
        Assert.Equal(WindowOptions.DefaultVulkan.WindowBorder, options.WindowBorder);
        Assert.True(options.IsVisible);
    }

    [Fact]
    public void OrdinaryStartupCreatesThePregameViewportBeforeShowingTheWindow()
    {
        ...
        Assert.Equal(new Vector2D<int>(800, 600), options.Size);
