...
{
    [Fact]
    public void ReturningToCharacterSelectionHidesGameplayNoticesBeforeTheViewportReflows()
    {
        var root = new UiRoot { Width = 1760f, Height = 990f };
        var state = new SpewBoxState();
        bool gameplay = true;
        using var controller = new SpewBoxController(root, new SpewBoxVM(state),
            isGameplayActive: () => gameplay);
        UiText text = Assert.Single(root.Children.OfType<UiText>());
        state.Enqueue("In Portal Space - Please Wait...");
        ...
        root.Width = 800f;
        root.Height = 600f;
        state.Enqueue("A late message from the previous world.");
        root.Tick(dt: 0d, nowMs: 1017L);
        Assert.False(text.Visible);
        Assert.Equal(0, state.Count);

        gameplay = true;
        root.Tick(dt: 0d, nowMs: 1018L);
        Assert.False(text.Visible);
        state.Enqueue("Welcome back.");
        root.Tick(dt: 0d, nowMs: 1019L);
        Assert.True(text.Visible);
        Assert.Equal("Welcome back.", Assert.Single(text.LinesProvider()).Text);
    }
