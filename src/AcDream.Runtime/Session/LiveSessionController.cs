...
public interface ILiveSessionOperations
{
    ...
    void BeginConnect(WorldSession session, string user, string password) =>
        session.BeginConnect(user, password);
    bool PollConnect(WorldSession session) => session.PollConnect();
      ...
      IRuntimeCharacterCreationCommands
{
        ...
        RuntimeGenerationToken generation)
    {
        ...
        public LiveSessionConnectOptions? ConnectingOptions { get; set; }
    ...
    private readonly RuntimeConnectionState _connection = new();
    ...
    public IRuntimeConnectionView Connection => _connection;
        ...
        lock (_gate)
        {
            ...
            if (_scope is not null)
            {
                return new LiveSessionStartResult(
                    _scope.ConnectingOptions is not null ? LiveSessionStartStatus.Deferred
    ...
    public void Tick()
    {
        lock (_gate)
        {
            ...
            RunTopLevel(() =>
            {
                ...
                if (scope.ConnectingOptions is { } connectingOptions)
                {
                    try
                    {
                        if (!_operations.PollConnect(scope.Session))
                            return;
                        scope.Session.ConnectionProgressChanged -= _connection.Apply;
                        scope.ConnectingOptions = null;
                        if (IsCurrent(scope, generation))
                            CompleteStart(scope, connectingOptions);
                    }
                    catch (Exception error)
                    {
                        Exception reportedError = StopAfterFailure(error);
                        _connection.Fail(error);
                        Console.Error.WriteLine($"live: connection failed: {reportedError}");
                    }
                    return;
                }
                try
        ...
        bool resetHost)
    {
        ...
        _connection.Apply(new(ConnectionPhase.Connecting));
        ...
        try
        {
            ...
            session.ConnectionProgressChanged += _connection.Apply;
            if (options.PollConnectionDuringTicks)
            {
                scope.ConnectingOptions = options;
                _operations.BeginConnect(session, options.User, options.Password);
                return new LiveSessionStartResult(LiveSessionStartStatus.Deferred);
            }
            try
            {
            ...
            if (!IsCurrent(scope, generation))
                return new LiveSessionStartResult(LiveSessionStartStatus.Deferred);
            return CompleteStart(scope, options);
        }
        catch (Exception startError)
        {
            Exception reportedError = StopAfterFailure(startError);
            _connection.Fail(startError);
            return new LiveSessionStartResult(
    ...
    private LiveSessionStartResult CompleteStart(SessionScope scope, LiveSessionConnectOptions options)
    {
        WorldSession session = scope.Session;
        ILiveSessionLifecycleHost host = scope.Host;
        ulong generation = scope.Generation.Value;
        LiveSessionBinding binding = scope.Binding!;
        _connection.Apply(new(ConnectionPhase.Ready));
