...
public sealed class MapHousePanelController : IRetainedPanelController
{
        ...
        ElementInfo rootInfo, ImportedLayout layout, Callbacks callbacks)
    {
        ...
        {
            Console.WriteLine(
                "[UI] MapHousePanelController.Bind: root did not build as UiTabPanel "
        ...
        if (layout.FindElement(CloseButtonId) is UiButton close)
            ...
            Console.WriteLine(
                $"[UI] MapHousePanelController: close button 0x{CloseButtonId:X8} not found.");
        ...
        if (mapPage is null)
            Console.WriteLine($"[UI] MapHousePanelController: Map page 0x{MapPageId:X8} not found.");
        if (housePage is null)
            Console.WriteLine($"[UI] MapHousePanelController: House page 0x{HousePageId:X8} not found.");
