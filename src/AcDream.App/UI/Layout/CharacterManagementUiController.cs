...
internal sealed class CharacterManagementUiController : IDisposable
{
    ...
    private bool _directLaunchPending;
        ...
        BitmapFont? versionFont)
    {
        ...
        _dialogs = dialogs;
        _bindings = bindings;
        _directLaunchPending = bindings.DirectCharacterLaunch;
        _strings = strings;
        ...
        var assembly = typeof(CharacterManagementUiController).Assembly;
        string version = assembly.GetCustomAttribute<AssemblyInformationalVersionAttribute>()
            ?.InformationalVersion.Split('+')[0]
            ?? assembly.GetName().Version?.ToString(3)
            ?? "unknown";
        UiText.Line[] versionLines =
            [new($"OpenAC {version}", new Vector4(0.65f, 0.65f, 0.65f, 1f))];
        Root.AddChild(new UiText
        {
            Name = "ClientVersion",
            Left = 8f,
            Top = 6f,
    ...
    internal void Tick(bool presentationBlocked = false)
    {
        if (_disposed)
            return;
        ...
        if (_bindings.DirectCharacterLaunch
            && snapshot.Lifecycle == RuntimeCharacterSelectionLifecycle.Connecting)
            _directLaunchPending = true;
        if (snapshot.Lifecycle == RuntimeCharacterSelectionLifecycle.InWorld
            || snapshot.Error is not null)
            _directLaunchPending = false;

        if (_presentationSuppressed || presentationBlocked || _directLaunchPending)
        {
            Deactivate();
            _lastRevision = long.MinValue;
            return;
