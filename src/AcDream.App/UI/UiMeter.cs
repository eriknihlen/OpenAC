...
public sealed class UiMeter : UiElement, IUiDatStateful
{
    ...
    private UiLoopingImageAnimation? _animatedBack;
    private UiLoopingImageAnimation? _animatedFront;
    private double? _animationStarted;

    internal void ConfigureAnimatedTracks(UiLoopingImageAnimation back, UiLoopingImageAnimation front)
    {
        _animatedBack = back;
        _animatedFront = front;
        BackTile = back.Sample(0d);
        FrontTile = front.Sample(0d);
        _animationStarted = null;
    }

    internal (uint Back, uint Front) SampleAnimatedTracks(double elapsedSeconds)
        => (_animatedBack?.Sample(elapsedSeconds) ?? BackTile,
            _animatedFront?.Sample(elapsedSeconds) ?? FrontTile);

    internal void AdvanceAnimatedTracks(double now)
    {
        if (_animatedBack is null || _animatedFront is null)
            return;
        if (_animationStarted is null || now - _animationStarted.Value >= _animatedBack.Duration)
            _animationStarted = now;
        (BackTile, FrontTile) = SampleAnimatedTracks(now - _animationStarted.Value);
    }
    ...
    {
        AdvanceAnimatedTracks(UiMediaClock.Seconds);
        float? pct = Fill();
