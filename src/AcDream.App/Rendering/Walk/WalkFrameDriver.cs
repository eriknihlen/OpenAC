...
{
    (RenderSceneGeneration Generation, uint TupleLandblockId) RetainedContext => default;
...
internal sealed class WalkFrameDriver : IWalkEventSink, IWalkLookInViewSource
{
    ...
    private readonly FarLandscapeDrawCache _farDrawCache;
        ...
        ClipFrame? clipFrame = null)
    {
        ...
        _farDrawCache = new FarLandscapeDrawCache(dispatcher, worldData);
    }
    ...
    internal void RebindFrame(
        ...
        ClipFrame? clipFrame)
    {
        AbortFrame(clearRetained: false);
        _leafRenderer = leafRenderer
            ?? throw new ArgumentNullException(nameof(leafRenderer));
        _clipFrame = clipFrame;
    }

    internal void AbortFrame(bool clearRetained = true)
    {
        if (clearRetained)
            _farDrawCache.Clear();
        _ctx = null;
        ...
        Vector3 cameraWorldPosition)
    {
        ...
        if (_ctx is not null)
        {
            throw new InvalidOperationException(
                ...
                + "the driver is not re-entrant; call "
        ...
        int cellIndex)
    {
        ...
        if (_landscapeViewRouteIndex < 0)
            throw new InvalidOperationException("A landscape turn requires an active view set.");
        if (_farDrawCache.TryAppend(
                blockPrefix, sideCellCount, cellIndex, _stream,
                this, _landscapeViewRouteIndex, _cameraWorldPosition,
                _alphaSubmissions))
        {
            MarkIfGrown();
            int cellSpan = 8 / sideCellCount;
            int startX = cellIndex / sideCellCount * cellSpan;
            int startY = cellIndex % sideCellCount * cellSpan;
            int alphaCellIndex = 0;
            for (int x = startX; x < startX + cellSpan; x++)
            for (int y = startY; y < startY + cellSpan; y++)
            {
                uint cellId = blockPrefix | (uint)(x * 8 + y + 1);
                VisitedLandscapeCellIds.Add(cellId);
                bool particles = _cellParticleTurnsDrawnThisFrame.Add(cellId);
                _alphaSubmitMark = _farDrawCache.AlphaEnds[alphaCellIndex++];
                _events.Add(WalkFrameEvent.LandscapeCellParticles(
                    cellId, _alphaSubmitMark, particles));
            }
            return;
        }
