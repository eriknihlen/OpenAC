...
namespace AcDream.App.Rendering.Wb
{
    ...
    public class ObjectMeshManager : IDisposable
    {
        ...
        internal bool UploadOrRequeue(MeshUploadQueueItem item)
        {
            ...
            _stagedMeshData.Complete(item);
            Console.WriteLine($"[up-retry] 0x{meshData.ObjectId:X10} upload failed {meshData.UploadAttempts}x — giving up (the upload error is being surfaced, not hidden)");
            return false;
        }
        ...
        public ObjectRenderData? UploadMeshData(ObjectMeshData meshData)
        {
            ...
            try
            {
                ...
                if (renderData == null)
                {
                    Console.WriteLine($"[up-null] 0x{meshData.ObjectId:X10} produced a 0-vertex mesh — caching empty render data (legitimate only for degenerate/no-valid-surface models)");
