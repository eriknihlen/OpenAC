...
public sealed unsafe partial class WbDrawDispatcher
{
    ...
    internal static List<OrderedMergeRun> BuildOrderedMergeRuns(
        ...
        List<OrderedMergeRun>? destination = null)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            if (stream.Stages[i] == WalkDrawStage.PortalPunch)
            {
                throw new NotSupportedException(
                    ...
                    + "which cannot be submitted through the ordered mesh path. "
                    + "Portal-punch geometry requires its dedicated emission path.");
            }
        }
        ...
        var runs = destination ?? new List<OrderedMergeRun>();
        runs.Clear();
        int cursor = 0;
        while (cursor < count)
        {
    ...
    internal static DrawElementsIndirectCommand OrderedIndirectCommand(
        GroupKey key, OrderedMergeRun instances) => new()
    {
        Count = (uint)key.IndexCount,
        InstanceCount = (uint)instances.CommandCount,
        FirstIndex = key.FirstIndex,
        BaseVertex = key.BaseVertex,
        BaseInstance = (uint)instances.FirstCommand,
    };
    ...
    private readonly List<OrderedMergeRun> _orderedInstanceRuns = new();
    private int[] _orderedIndirectOffsets = Array.Empty<int>();
        ...
        IReadOnlyList<int>? forcedBreaksAscending = null)
    {
        ...
        {
            OrderedMergeRun instances = _orderedInstanceRuns[i];
            GroupKey key = stream.Keys[instances.FirstCommand];
            int end = instances.FirstCommand + instances.CommandCount;
            for (int command = instances.FirstCommand; command < end; command++)
                _orderedIndirectOffsets[command] = i;
            ...
            _indirectCommands[i] = OrderedIndirectCommand(key, instances);
        ...
        _orderedCommands = new RhiSection(
            ...
            checked((uint)(drawCount * DrawCommandStride)));
