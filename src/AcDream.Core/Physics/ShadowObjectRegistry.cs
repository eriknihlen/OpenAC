...
public sealed class ShadowObjectRegistry
{
    ...
    private readonly ConditionalWeakTable<List<RetailPartEntry>, CellRenderStamp> _cellRenderStamps = new();
    private ulong _nextCellRenderRevision;

    private sealed class CellRenderStamp
    {
        internal ulong Revision;
    }

    public ulong GetCellRenderRevision(uint cellId)
    {
        if (!_retailPartEntriesByCell.TryGetValue(cellId, out var entries) || entries.Count == 0)
            return 0;
        if (!_cellRenderStamps.TryGetValue(entries, out var stamp))
        {
            stamp = new CellRenderStamp { Revision = checked(++_nextCellRenderRevision) };
            _cellRenderStamps.Add(entries, stamp);
        }
        return stamp.Revision;
    }

    private void MarkCellRenderChanged(List<RetailPartEntry> entries)
    {
        if (_cellRenderStamps.TryGetValue(entries, out var stamp))
            stamp.Revision = checked(++_nextCellRenderRevision);
    }

    private void MarkOwnerRenderChanged(uint entityId)
    {
        if (!_retailCellArrays.TryGetValue(entityId, out var cells))
            return;
        foreach (uint cellId in cells)
        {
            if (_retailPartEntriesByCell.TryGetValue(cellId, out var entries))
                MarkCellRenderChanged(entries);
        }
    }
    ...
    private void BumpOwnerVersion(uint entityId)
    {
        ...
        MarkOwnerRenderChanged(entityId);
    ...
    private void RemoveOwnerPartRows(
        ...
        out SetPositionShadowCommitReceipt receipt)
    {
        ...
        MarkOwnerRenderChanged(prepared.EntityId);
        _lastAppliedSetPositionCommitId = prepared.CommitId;
        _pendingSetPositionDispatches.Add(prepared.CommitId);
